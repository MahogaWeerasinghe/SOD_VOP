...
if __name__ == "__main__":
    # datasets = ['ECSSD', 'HKUIS', 'PASCAL', 'DUT-OMRON', 'THUR15K', 'DUTS-TEST', 'SOD']
    datasets = ['DCN', 'ECSSD']
    # methods = ['EGNet-R', 'BASNet', 'AFNet', 'CPD-R', 'PiCANet-R', 'RANet', 'DGRL', 'PAGR', 'R3Net', 'SRM',
    #            'Amulet', 'BMPM']
    # methods = ['R3Net', 'SRM', 'Amulet', 'BMPM']
    methods = ['DCN']
    for method in methods:
        for dataset in datasets:
            pred_path = '/content/drive/MyDrive/Mahoga/Results/' + method + '/' + dataset + '/'
            gt_path = '/backup/materials/Dataset/SalientObject/dataset/' + dataset + '/gts/'
            if not os.path.exists(pred_path):
                continue
            loader = EvalDataset(img_root=pred_path, label_root=gt_path)
            if len(loader.image_path) == 0 or len(loader.image_path) != len(loader.label_path):
                continue
            output_dir = './scores/' + method + '/' + dataset
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            evaluater = Eval_thread(loader=loader, method=method, dataset=dataset, output_dir=output_dir)
            evaluater.run()
